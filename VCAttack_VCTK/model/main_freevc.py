...
from model_freevc import Discriminator, Generator
from train_freevc import train_noise_encoder
import os
import sys
#sys.path.append('/root/VCAttack_VCTK')
...
#wav2vec = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-large-960h-lv60-self").to(device)
freevc_encoder = SpeakerEncoder('/root/VCAttack_VCTK/FreeVC/speaker_encoder/ckpt/pretrained_bak_5805000.pt').to(device)
...
# 학습 실행
print("Train VCTK dataset wav2vec")
img_losses, emb_losses, gan_losses = train_noise_encoder(generator, discriminator, freevc_encoder, train_loader, optimizer_G, optimizer_D, num_epochs, batch_size, device, lambda_wav, lambda_emb)
