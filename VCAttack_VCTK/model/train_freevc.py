...
import wandb
# wandb 초기화
wandb.init(project="VCAttack_VCTK")

import sys
sys.path.append('/root/VCAttack_VCTK/FreeVC')
...
def train_noise_encoder(generator, discriminator, extractor, dataloader, optimizer_G, optimizer_D, num_epochs, batch_size, device, lambda_wav, lambda_emb):
    ...
    for epoch in range(num_epochs):
        ...
        for waveforms in tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            validity_fake = discriminator(noisy_mel_spectrograms.reshape(batch_size, 1, -1))  # Generator 결과만 사용
            ...
            # 임베딩 손실 부분 수정
            with torch.no_grad():                
                # forward pass로 임베딩 추출
                mel_orig = mel_spectrograms.squeeze(1).transpose(1, 2)  # [batch, time, freq]
                mel_noisy = noisy_mel_spectrograms.squeeze(1).transpose(1, 2)
                extractor_original_output = extractor(mel_orig)
                extractor_noisy_output = extractor(mel_noisy)
            ...
            validity_real = discriminator(mel_spectrograms.detach().reshape(batch_size, 1, -1))  # detach()로 그래프 끊기
            validity_fake = discriminator(noisy_mel_spectrograms.detach().reshape(batch_size, 1, -1))  # detach()로 그래프 끊기
