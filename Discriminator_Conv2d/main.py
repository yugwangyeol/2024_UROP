...
lambda_wav = 0.8
# 데이터 로드
train_dataset = torchaudio.datasets.VCTK_092(root="../../Data", download=False)
def collate_fn(batch):
    waveforms = []
    max_length = max(waveform.shape[1] for waveform, sample_rate, utterance, speaker_id, utterance_id in batch)
    for waveform, sample_rate, utterance, speaker_id, utterance_id in batch:
...
# 학습 실행
print("train Discriminator conv2d")
