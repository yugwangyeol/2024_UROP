...
def main():
    ...
    parser.add_argument('--model_type', type=str, choices=['wavlm', 'hubert'], default='wavlm',
                      help='Type of model to use (wavlm or hubert)')
    parser.add_argument('--attack_type', type=str, choices=['white', 'black'], default='white',
                      help='Type of attack (white-box or black-box)')
    args = parser.parse_args()

    # 모델 경로 설정
    model_path = f"model/checkpoints/generator_{args.model_type}.pth"

    # attack_type을 w/b로 축약
    attack_abbr = 'w' if args.attack_type == 'white' else 'b'

    # 입출력 경로 설정
    input_pairs_file = "data/Original_style.txt"
    output_pairs_file = f"data/FreeVC_test_noisy_pairs_{attack_abbr.upper()}.txt"
    output_wav_dir = f"data/FreeVC_noisy_style_{attack_abbr.upper()}"
